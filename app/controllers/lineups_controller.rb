class LineupsController < ApplicationController
  ...
  def create
    @festival = Festival.find(params[:festival_id])
    @lineup = @festival.lineups.create(festival_id: params[:festival_id], film_id: params[:film_id], year: params[:year])

    respond_to do |format|
      if @lineup.save
        format.html { redirect_to festival_lineup_path(@festival), notice: 'Lineup was successfully created.' }
